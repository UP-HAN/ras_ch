...
운영 데이터를 리셋해도 시연용 데이터는 따로 살아 있게, 같은 서버에 **두 번째 인스턴스**를 둔다: 디렉터리 `/var/app/ras-demo`, DB `ras_demo`, pm2 `ras-demo`(3301), 주소 `https://ras1.ches.es.kr`(검색 색인 금지). 화면에는 시연용이라는 표시가 전혀 없고(주소도 ras1), 운영 사이트 **관리자 메뉴에만** 링크(🎭 시연 사이트)가 보인다(운영 `.env` 의 `DEMO_SITE_URL`).
...
cp /var/app/ras/.env /var/app/ras-demo/.env   # PORT=3301, DB_NAME=ras_demo, UPLOAD_DIR=/var/app/ras-demo/server/uploads, CLIENT_ORIGIN=https://ras1.ches.es.kr 로 수정
...
sudo cp deploy/nginx/ras1.ches.es.kr.conf /etc/nginx/sites-available/ras1.ches.es.kr
sudo ln -s /etc/nginx/sites-available/ras1.ches.es.kr /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx && sudo certbot --nginx -d ras1.ches.es.kr
```
