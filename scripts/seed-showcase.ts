...
/** 댓글 말투 3단계 */
const COMMENTS_HIGH = [
  '자동 재생 끄기 정말 좋은 방법이에요! 저는 알림까지 껐더니 훨씬 덜 보게 됐어요. 다음 주도 같이 지켜요.',
  '타이머 울리면 바로 끄는 거 진짜 대단해요. 저는 아직 5분만 더 하다가 늦어져요. 비결이 뭐예요?',
  '할머니 댁에서 별 본 이야기 읽으니까 저도 폰 내려놓고 밖에 나가고 싶어졌어요. 다음 주 목표도 응원해요!',
  '사용시간이 늘었어도 솔직하게 쓴 게 멋져요. 저도 게임 전에 알람 맞추는 거 따라 해 볼게요.',
  '방해금지 모드는 설정에서 켤 수 있어요! 저는 공부 시간에만 켜 두는데 도움이 많이 돼요.',
  '운동하면 폰 생각이 덜 난다는 말 완전 공감해요. 저도 줄넘기 다시 시작해야겠어요. 화이팅!',
  '동생이랑 내기하는 거 재밌겠어요. 저도 이번 주말에 가족이랑 해 볼게요. 결과 알려 드릴게요!',
  '그림일기 아이디어 좋아요. 폰 대신 손으로 무언가 만드는 시간이 더 기억에 남는 것 같아요.',
];
const COMMENTS_MID = [
  ...
  '밤 10시에 폰 맡기기 좋은 생각이에요. 저도 해 볼게요.',
  '축구 연습 열심히 하는 거 멋져요. 응원해요!',
];
const COMMENTS_LOW = [
  '멋져요! 다음 주도 화이팅!',
  '나도 해 볼래요. 응원해요.',
...
const OPINIONS_AGREE = [
  ...
  '내 생각에는 찬성이에요. 규칙이 있어야 다 같이 지킬 수 있으니까요.',
];
const OPINIONS_DISAGREE = [
  '나는 반대해요. 왜냐하면 급한 일이 생기면 부모님께 연락해야 하기 때문이에요.',
  '나는 반대해요. 왜냐하면 스스로 조절하는 연습이 더 중요하다고 생각하기 때문이에요.',
  '내 생각에는 반대예요. 억지로 못 하게 하면 몰래 하게 될 것 같아요.',
];
const OPINIONS_OPEN = [
  '내 생각에는 학교에 폰 보관함을 두면 좋겠어요. 아침에 넣고 집에 갈 때 찾으면 되니까요.',
  '나는 폰 대신 보드게임을 교실에 두면 쉬는 시간이 더 재밌어질 것 같아요.',
  '친구들과 정한 규칙을 칠판에 붙여 두면 서로 지키기 쉬울 것 같아요.',
  '일주일에 하루는 폰 없는 날로 정하면 좋겠어요. 그날은 운동장에서 다 같이 놀아요.',
];
const ARTICLES = [
  {
...
async function main(): Promise<void> {
  ...
  for (const [wi, wk] of [...WEEKS, CUR].entries()) {
    ...
    for (const p of personas) {
      ...
      const texts =
        p.level === 'high' ? COMMENTS_HIGH : p.level === 'mid' ? COMMENTS_MID : COMMENTS_LOW;
      for (let k = 0; k < nC; k += 1) {
        ...
        const id = await comment(p.u, 'post', t.id, pickR(texts), at, 'COMMENT_WRITTEN');
  ...
  for (const wk of [...WEEKS, CUR]) {
    ...
    for (const p of personas) {
      ...
      if (t.type === 'vote') {
        const side = chance(0.6) ? 'agree' : 'disagree';
        ...
        if (chance(p.level === 'high' ? 0.7 : 0.35)) {
          const id = await comment(
            p.u,
            'news_topic',
            t.id,
            side === 'agree' ? pickR(OPINIONS_AGREE) : pickR(OPINIONS_DISAGREE),
            at.add(10, 'minute'),
            'NEWS_OPINION',
          );
          opinionIds.push(id);
        ...
        opinionIds.push(
          await comment(p.u, 'news_topic', t.id, pickR(OPINIONS_OPEN), at, 'NEWS_OPINION'),
        );
      }
  ...
  if (co1 && co2) {
    ...
    const notice = await mk(
      co1,
      {
        ...
        body: '10월 첫째 주부터 2주 동안 "자기 전 폰 거실에 두기" 챌린지를 해요. 참여하는 반은 교실 뒤 게시판에 스티커를 붙여요. 다 같이 해 보면 생각보다 쉬워요! 궁금한 건 자치회 임원에게 물어봐 주세요.',
    ...
    const poll = await mk(
      co2,
      {
        ...
        body: '10월 가을 축제에서 6학년이 맡을 부스를 정해요. 친구들이 가장 원하는 것으로 정할게요. 한 번 고르면 바꿀 수 없으니 잘 생각해서 골라 주세요!',
    ...
    await mk(
      co1,
      {
        ...
        body: '자치회에서 반마다 보드게임을 빌려줘요. 점심시간에 자치회실로 오면 루미큐브, 할리갈리, 젠가를 빌릴 수 있어요. 쉬는 시간에 폰 대신 친구들과 놀아요!',
    ...
    await mk(
      co2,
      {
        type: 'report',
        title: '9월 자치회 활동 보고',
        body: '9월에는 폰 없는 하루 챌린지를 두 번 했고 보드게임 대여를 시작했어요. 챌린지에 6학년 180명이 참여했어요. 10월에는 가을 축제 부스와 폰프리 챌린지를 준비할게요. 의견이 있으면 자치회 임원에게 말해 주세요.',
  ...
  await createNotice(
    { id: admin.row.id },
    {
      ...
      body: '10월 첫째 주부터 자기 전 폰 거실에 두기 챌린지를 해요. 리포트에 실천한 날을 적어 주세요.',
  ...
  await createNotice(
    { id: admin.row.id },
    {
      ...
      body: '폰 사용시간 캡처 2장과 성찰 글 100자 이상! 어려우면 담임 선생님께 물어보세요.',
