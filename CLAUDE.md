...
9. **존댓말 원칙(2026-09-25 사용자 결정)**: 이 사이트의 모든 글·댓글·기사·토론 의견·자치회 글은 **존댓말**로 쓴다. 화면의 안내 문구·문장 도우미·시드/더미 데이터·예시 문장도 전부 존댓말. 글쓰기·댓글 화면에는 `PoliteNotice`/`PoliteWarning`(lib/politeness)로 원칙을 보여 주고 반말 문장을 부드럽게 알려 준다(차단하지 않음). 교사는 반려 사유 `not_polite`, 임원은 체크리스트 `polite`로 확인한다. 새 문구를 만들 때 반말 어미(~다, ~야, ~자)를 쓰지 않는다.
